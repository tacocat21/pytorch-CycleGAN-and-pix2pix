...
class EvolutionaryCycleGANModel(BaseModel):
    ...
    def compute_visuals(self, epoch=None):
        ...
        if epoch is None:
            ...
            vutils.save_image(fake_A_display, os.path.join(self.save_dir, self.train_outputs_dir, '{}_fake_A.png'.format(epoch)), normalize=True, nrow=6)
            vutils.save_image(fake_B_display, os.path.join(self.save_dir, self.train_outputs_dir, '{}_fake_B.png'.format(epoch)), normalize=True, nrow=6)

            vutils.save_image(self.real_A, os.path.join(self.save_dir, self.train_outputs_dir, '{}_real_A.png'.format(epoch)), normalize=True, nrow=6)
            vutils.save_image(self.real_B, os.path.join(self.save_dir, self.train_outputs_dir, '{}_real_B.png'.format(epoch)), normalize=True, nrow=6)
